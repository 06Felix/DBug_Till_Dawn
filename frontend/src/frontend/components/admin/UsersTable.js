...
const UsersTable = () => {
  ...
  return (
    <div className="unique-report-container">
      <h3 className="unique-report-title mb-3">User Reports</h3>
      ...
      <table className="unique-report-table table-bordered table-hover">
        ...
        <tbody>
          {loading ? ( // If loading, display loading message
          ...
          ) : userData.length === 0 ? ( // If no data, show 'No users found' message
            <tr>
            ...
            userData.map((user, index) => (
              <tr key={user.id}>
                <td>{index + 1}</td>
                <td>{user.email}</td>
                <td>{new Date(user.lastLoggedInTime).toLocaleString()}</td>
              </tr>
            ))
          )}
        </tbody>
